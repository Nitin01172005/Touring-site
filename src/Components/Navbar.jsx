...
const Navbar = () => {
  ...
  useEffect(() => {
      if(showMobileMenu){
        document.body.style.overflow= 'hidden'; //doesn't allow scrolling
      }
      else {
  ...
  return (
    <div className='top-0 left-0 absolute w-full z-10'>
      <div className='container mx-auto flex justify-between items-center py-4 px-6 md:px-20 lg:px-32'>
        ...
        <img onClick={()=> setShowMobileMenu(true)} src={assets.menu_icon} className='md:hidden w-7 cursor-pointer' alt="Menubar" /> 
      </div>
      {/* -----monile menu ------*/}
      <div className={`md:hidden ${showMobileMenu ? 'fixed w-full' : 'h-0 w-0' } fixed w-full right-0 top-0 bottom-0 overflow-hidden transition-all bg-white`}>
        <div className='flex justify-end p-6 cursor-pointer'>
          <img onClick={() => setShowMobileMenu(false)}  src={assets.cross_icon}  className='w-6' />
        </div>
        <ul className='flex flex-col items-center py-0 mt-5 px-5 text-lg font-medium'>
